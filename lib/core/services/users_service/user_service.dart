...
class UsersService {
  static Future<GenericModel> updateProfile({
    ...
    try {
      ...
      var request = http.MultipartRequest('POST', Uri.parse('${ENV.baseURL}/api/profile?_method=patch'));
      ...
      if (pickedAvatar != null) {
        request.files.add(await http.MultipartFile.fromPath('avatar', pickedAvatar.path));
      }
      ...
      if (response.statusCode == 200) {
        ...
        return GenericModel(
            success: false, returnedModel: null, message: "Session Expired", title: "Please Login again");
      } else {
  ...
  static Future<GenericModel> updatePassword(
      {required String currentPass, required String newPass, required String token}) async {
    try {
      var headers = {'Accept': 'application/json', 'Authorization': "Bearer $token"};

      var request = http.Request(
          'PATCH', Uri.parse('${ENV.baseURL}/api/password?password=$newPass&current_password=$currentPass'));
      ...
      if (response.statusCode == 200) {
        return GenericModel(
            success: true, returnedModel: null, title: "Password Updated", message: "Password Updated Successfully");
      } else if (response.statusCode == 422) {
        ExceptionModel exceptionModel = ExceptionModel.fromJson(jsonDecode(res));
        return GenericModel(
            success: false,
  ...
  static Future<GenericModel> logout({
    ...
    try {
      ...
      var request = http.MultipartRequest('POST', Uri.parse('${ENV.baseURL}/api/logout'));
      request.headers.addAll(headers);
      http.StreamedResponse response = await request.send();
      var res = await response.stream.bytesToString();
      print(res);
  ...
  static Future<GenericModel> getUsers({required String token, required int page, String? type}) async {
    try {
      print("AUTH TOKEN $token $type");
      var headers = {'Accept': 'application/json', 'Authorization': "Bearer $token"};
      var request = http.Request('GET', Uri.parse('${ENV.baseURL}/api/users?filters[type]=$type&page=$page'));
      ...
      if (response.statusCode == 200) {
        ...
        return GenericModel(returnedModel: usersModel, success: true, statusCode: response.statusCode);
      } else if (response.statusCode == 401) {
        LogicHelper.unauthorizedHandler(showMessage: true);
        return GenericModel(returnedModel: null, success: false, statusCode: response.statusCode);
      } else {
        ///TODO
        print(response.reasonPhrase);
        return GenericModel(returnedModel: null, success: false, statusCode: response.statusCode);
  ...
  static Future<GenericModel> addUser({required String token, required RegistratingData registratingData}) async {
    try {
      var headers = {
        'Accept': 'application/json',
        'Authorization': "Bearer $token" //'Bearer 17|eOHvmIaVstztJRPkBLM1xsw5XURqnx2Z6ctWxtz1'
      };

      var request = http.MultipartRequest('POST', Uri.parse('${ENV.baseURL}/api/users'));
      request.fields.addAll(registratingData.toJson());
      request.headers.addAll(headers);
      http.StreamedResponse response = await request.send();
      var res = await response.stream.bytesToString();
      if (response.statusCode == 200) {
        UserRoleModel usersModel = UserRoleModel.fromJson(jsonDecode(res)['data']);
        return GenericModel(
          success: true,
          returnedModel: usersModel,
          message: "${registratingData.name} will received e-mail containing login credentials",
        ...
        return GenericModel(
            success: false, title: "Member Added", message: "Member Added Successfully", returnedModel: null);
      }
    } on SocketException {
      return GenericModel(
          returnedModel: null,
  ...
  Future<GenericModel> getUserRoles(
      {required String accessToken, required int page, required role, int? projectId, String? searchQuery}) async {
    try {
      ...
      print(res);
      if (response.statusCode == 200) {
        return GenericModel(returnedModel: UserRoleResponse.fromJson(jsonDecode(res)), success: true, statusCode: 200);
      } else if (response.statusCode == 422) {
        ExceptionModel exceptionModel = ExceptionModel.fromJson(jsonDecode(res));
        return GenericModel(
