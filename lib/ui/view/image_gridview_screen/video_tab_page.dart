...
class VideosTabBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Consumer<LoadedProjectProvider>(builder: (ctx, loadedProjectProvider, c) {
      if (loadedProjectProvider.getLoadedProject == null || mediaObject.videos == null || mediaObject.videos!.isEmpty) {
        return const Center(
            ...
            return InkWell(
              onTap: () {
                Navigator.of(context)
                    .push(MaterialPageRoute(builder: (_) => VideoPreview(videos: mediaObject.videos!, index: index)));
              },
              child: Stack(
                alignment: Alignment.center,
                children: [
                  CustomCachedImage(
                      imageUrl: mediaObject.videos![index].thumbnailPath, fit: BoxFit.cover, width: 100.w),
                  const Icon(Icons.play_circle_filled_outlined, size: 38)
                ],
              ),
            );
          });
